...
from tests.resources.tree_examples import TREE_TWO_DEP_ONE_INDEP, TREE_THREE_DEP_ONE_INDEP
...
def test_three_features_tree_groupshap_values():
    ...
    np.testing.assert_equal(explainer.shap_values([1,1,1], feature_reprs=[1,1,2]), [0,3/4,5])

    np.testing.assert_equal(explainer.shap_values([1,0,1], [0,1,1]), [0.125,4.625,0])
...
def test_four_features_tree_groupshap_values():
    ...
    np.testing.assert_equal(explainer.shap_values([1,1,1,1], [0,0,0,3]), [0.875,0,0,5])
    np.testing.assert_almost_equal(explainer.shap_values([1,1,1,1], [0,1,2,3]), [0.875/3,0.875/3,0.875/3,5], decimal=6)

    # Check that groupSHAP do not equal groupwise sum-of-shap when there are interactions across groups
    np.testing.assert_equal(explainer.shap_values([1,1,1,1], [0,0,2,3]), [0.5625, 0, 0.3125, 5])
    with pytest.raises(Exception):
        np.testing.assert_almost_equal(explainer.shap_values([1,1,1,1], [0,1,2,3]), [0.5625/2, 0.5625/2, 0.3125, 5], decimal=2)
...
def test_init_with_model_get_booster():
    ...
    with patch("grouptreeshap.explainer.TreeEnsemble.from_xgboost", return_value=ensemble) as m:
