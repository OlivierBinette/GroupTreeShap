...
def read_element(ubj: BytesIO, marker: bytes, type_spec: tuple | None = None) -> JSONLike:
...
def read_numeric(ubj: BytesIO, marker: bytes) -> int | float:
    match marker:
        ...
        case _:
            raise UBJSONDecodeError(f"Invalid numeric marker {marker} encountered at position {ubj.tell()}")
...
def read_container(ubj: BytesIO, marker: bytes, type_spec: None | tuple) -> list[JSONLike] | dict[str, JSONLike]:
...
def read_list_unoptimized(ubj: BytesIO, count: int | None, type_spec: None | tuple = None) -> list[JSONLike]:
    container = list()
    if count is None:
...
def read_list_optimized(ubj: BytesIO, type_markers: list[bytes], counts: list[int]) -> list[JSONLike]:
    container = list()
...
def read_list(ubj: BytesIO, type_markers: list[bytes], counts: list[int]) -> list[JSONLike]:
...
def read_dict_unoptimized(ubj: BytesIO, count: int | None, type_spec: None | tuple = None) -> dict[str, JSONLike]:
    container = dict()
    if count is None:
        while (marker := get_next_marker(ubj)) != b'}':
            ubj.seek(-1, SEEK_CUR)
